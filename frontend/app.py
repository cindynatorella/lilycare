...
def create_app() -> Flask:
    ...
    def update_profile():
        state = _load_state()
        state["profile"]["birth_date"] = request.form.get("birth_date", "").strip()
        state["profile"]["description"] = request.form.get("description", "").strip()
        _save_state(state)
        flash("Lily's profile has been updated for this demo session.", "success")
